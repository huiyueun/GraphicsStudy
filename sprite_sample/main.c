...
main(int argc, char **argv)
{
	...
	ad.window = SDL_CreateWindow(NULL, 600, 0, WINDOW_WIDTH, WINDOW_HEIGHT, SDL_WINDOW_SHOWN | SDL_WINDOW_OPENGL);
	if (ad.window == 0) {
			SDL_Quit();
			return -1;
	}
    ...
    while(!isQuit) {
        ...
        if ((SDL_GetTicks() - lastTime) > 1000 / FPS )
        {
            ...
            SDL_FillRect(ad.window_surface, NULL, SDL_MapRGB(ad.window_surface->format, 0, 0, 0));
